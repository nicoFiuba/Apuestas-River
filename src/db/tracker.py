...
def record_bet(match_name: str, selection: str, bookmaker: str, odds: float, ev_percentage: float, stake_ars: float) -> int:
    try:
        ...
        logging.error(f"Error al registrar apuesta en Supabase: {e}")
        return -1
...
def resolve_bet(bet_id: int, status: str) -> bool:
    try:
        ...
        logging.error(f"Error al resolver apuesta en Supabase: {e}")
        return False
...
def get_pending_bets() -> list:
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, match_name, selection, bookmaker, odds, stake_ars, created_at 
            FROM bets 
        ...
        logging.error(f"Error al consultar apuestas pendientes: {e}")
        return []
...
def get_performance_summary() -> dict:
    try:
        ...
        logging.error(f"Error al obtener resumen de rendimiento: {e}")
...
def save_odds_history(match_name: str, bookmaker: str, odds: float) -> str:
    try:
        ...
        logging.error(f"Error al guardar historial de cuotas en Supabase: {e}")
        return None
